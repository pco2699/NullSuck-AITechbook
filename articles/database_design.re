...
たとえば、「お金を返す人／返さない人」を自動的に判断することによって業務を効率化したいときに用います。
...
ここまで、目的を達成するためのモデル全体像とそれを実現するための
分析手法、アルゴリズムを検討しました。

=== 仮説を立てる（主観）
実際にデータを見る前に、しっかりと仮説を立てましょう。
...
データセットは@<href>{http://archive.ics.uci.edu/ml/machine-learning-databases/wine-quality/}から無料でダウンロードできます。
...
そのうえで、どのようにデータを集めていくのかを検討する必要があります。
...
データが汚いという場合は、1~4の中にあいまいな部分や不足箇所が多いということです。
...
数値を見て分かることは@<kw>{テーブル表}で十分です。
テーブル表でも、数値の意味を理解することが難しいときに@<kw>{グラフによる可視化}を行います。

それでは、1~6までのステップを最短経路で走りきってみましょう！

==== 役割ごとにディレクトリを作成する
まずは、analyticsというディレクトリを作成してください。
つぎに、3つのディレクトリを作成します。分析作業は、/workspace で行います。
...
;（セミコロン）で列に分割できそうですので、sep=';'を追記して列に分割します。
...
//image[analytics04][列名を貼り付ける][scale=0.5]

すると、「自動的に列に分割」と表示されるのでクリックしてください。
基本的に空白やカンマ、セミコロンがあれば、きれいに分割できます。

//image[analytics05][列に分割された][scale=0.5]
...
算出自体はコードを1行書くだけですので、なぜこの1行を書いて数値を見る必要があるのかについて
簡単に解説します。
...
たとえば、あなたはいつも500円のランチを食べているとしましょう。500円が平均ランチ代です。
...
期待値・平均値は、その値がどれだけすごいのか（普通or珍しい）を一概に判断することはできません。
そこで活躍するのが@<kw>{（標準）偏差}です。
標準偏差はある値と平均の差が@<kw>{どれだけすごいのか}を把握するために使います。
そして、分散はばらつきの大きさを表現しています。

第2章でも書きましたが、モデルを作るためのデータセットは
...
数値データの場合は、いったんカテゴリデータに変換して集計することが多いです。

今回、美味しさは列名「quality」= ワインの品質の値と定義します。
そのヒストグラムを見てみましょう。

//image[analytics10][qualityのヒストグラム][scale=0.7]
...
ちなみに、全体をひとくくりで分析するのは大事な視点を見逃してしまう可能性が高いです。
...
//table[envvars][数値データからカテゴリデータをつくる例]{
...
@<kw>{隙間がなく、規則性がある}機械学習用のデータセットを作ることができました。
...
その大きさを数値で詳しく見てみます。そこで登場するのが@<kw>{相関係数}です。
相関係数は、AとBがどれくらい相関しているかを表す数値です

==== 相関係数の見方
一言でいうと、@<kw>{正なら正。負ならば負。}です。
...
====[column] 要因が分かるだけでは、世の中は何も変わらない
データ分析をするときに、AとBの相関関係を発見することはとてもおもしろいです。
@<kw>{おむつとビール}の話がわかりやすい事例です。

もし、あなたが飲料メーカーで働いているとします。
...
いったんモデルを構築したあとに、分析手法や説明変数の見直しの機会を設けることは大切です。
そして、いきなり未来予測をせずに、正解がわかっている現在手元にあるデータをもとに仮の予測を行い、
...
dumpされたモデルファイルが次のとおり保存されていれば完了です。
...
ユーザーにもっともインパクトを残したい画面から作るようにしています。
...
あるいは、採用した@<kw>{アルゴリズムが不適切}である場合もあるでしょう。@<br>{}

　私はもうひとつ理由があることに気付きました。
ヒトは絶対に当たる予測モデルが作れないことを知ると、予測結果ではなく@<kw>{説明できる根拠}を知りたがるから、
ビジネス実装することが難しいのです。

　たとえば、DeepLarningを使って予測精度の高いモデルを構築しても、
その予測モデルを求めていた当事者が@<kw>{予測の根拠}を知りたがるのです。
しかし、機械学習アルゴリズムにおいて@<kw>{予測精度の高さと説明性の高さは絶対的な正の相関関係があるわけではありません。}@<br>{}

もし、あなたが@<kw>{的中率100%で噂の占い師に「彼が、あなたを幸せにしてくれる運命の人です。結婚しなさい。」}といわれても、
