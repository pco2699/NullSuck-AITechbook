...
#@# ↑この、本章では〜の始まり方、死ぬほど良い
#@# できれば１章２章もあると嬉しい。序盤ということもあり。
...
==== jupyterを起動する
#@# jupyter知らないエンジニアのためにどっかで一言言及したほうが良いかも
...
最短経路のデザイン方法のポイントは5つです。

 1. 見本を手に入れろ！
 2. コアデザインを見極めろ！
 3. 手書きで書き起こせ！
 3. UIフレームワークを使いこなせ！
 4. AdobeXDのコンポーネントを使おう！

=== AIアプリの設計仕様
==== 教師あり学習の仕組み
//image[aiapp01][モデリングの図解]

==== AIアプリ化
//image[aiapp02][アプリのAI化]

=== 最小の技術で最速の画面設計


//image[ui01][画面作成のステップ]
==== 見本の入手
これまで芝麻信用について詳しく書いてきたので、今回は芝麻信用の画面デザインを見本にしましょう。
芝麻信用のUIは図の通りです。ヘッダーがあり、メインメニューにスコアをレーダーチャートによって可視化しています。

==== コアデザイン
デザイナーがいない場合は、その画面のどの要素を取り入れたいのかを整理してピックアップします。



作成する画面は2画面ですが、ユーザーに伝えたいのは「予測結果の表示」なので、予測結果の表示画面からデザインします。

==== デザインキットの有効活用
デザインツールはadobe社のXDを使います。
無償版が提供されており、無料のデザインキットが提供されているため個人開発に最適です。

AdobeXDはチュートリアルが豊富であるため、本書では詳しい解説は割愛します。

//image[ui02][デザインキットによる効率化]
