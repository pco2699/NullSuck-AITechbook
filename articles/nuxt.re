...
== Nuxt.jsとは
Nuxt.jsとは、Vue.jsアプリケーションを作るためのフレームワークです。
Vue.jsもフレームワークですので、フレームワークを作るためをフレームワーク、ということになります。
...
どうしてもコードが汚くなったり、複雑な動きの実装が難しくなってしまいました。

そこで、出てきたのがフロントエンドフレームワークです。フロントエンドフレームワークは、様々な手法で先の問題を解決することを試みています。
近年だと AngularJS/React/Vue.jsが有名です。Vue.jsはその中でも、HTML5/CSS3などの知識を活かせて学習コストが低いフレームワークとして近年人気が出ています。

=== Nuxt.js
そして、そのVue.jsのフレームワークとして出てきたのがNuxt.jsです。
Vue.jsそのものは単純にビューのフレームワークであるため、環境構築のしかたや使い方などは実は非常に多様でした。
...
しかし、個人的には、上記、述べたとおりVue.jsに必要なミドルウェアやプロジェクト構成がすべて用意されている、というのがNuxt.jsのメリットです。

たとえば、Nuxt.jsを使わずに、Vue.jsを使おうとするとwebpackというフロントエンドで使われるライブラリの知識が必ず必要になります。@<fn>{webpack}
...
@<code>{template}, @<code>{style}, @<code>{script} の順番はどの順番でもよいですし、
...
診断画面は質問数のぶんだけカードが並んでいます。
今回は合計で11コの質問があるので、11コカードが並んでいる形です。

これを普通にHTMLなどで書くとコピー&ペーストの嵐となっていますが
Nuxt.js/Vue.jsだとプログラマブルに、冗長なく書けるのがうれしいところです。

こういった繰り返しの部品は「コンポーネント」として作成します。
この節では、診断画面のカードを作ることでコンポーネントの作り方を学びます。
...
FormCardもさきほどと同様にすでにあるVuetifyのUIコンポーネントを組み合わせて作ることができます。
...
//image[question_screen_vue][診断カードのソースコードと画面の紐づけ]
...
上記のとおり書くことで、診断カードを11コ表示させることができます。
注意点は次のとおりです。

 * この書き方はTypeScipt/ES6のクラス構文ベースになっています。
   ほかの本やページなどでは、別の書き方をしているかもしれませんが
   それでももちろん動きます。
...
表示に関するデータを、コンポーネントごとに管理すると面倒くさいよねー、それなら、ひとつの場所にまとめてしまおう！
というのがVuexの考え方だと思っています。

このひとつの場所、というのがStoreおよびStateと呼ばれる場所になります。

=== Stateを定義しよう
では、Stateを実際に定義してみましょう。
Stateは、先ほど説明したディレクトリの@<tt>{store}の配下にファイルを作ると
...
Stateはさきほども書いたとおり、アプリ中すべてのコンポーネントから使われるデータなので
いろいろなデータをもつ必要があります。
...
TypeScriptのinterfaceを使うと事前に定義しておくことができます。

これがNuxt.jsでTypeScriptを利用する一番のメリットではないか、と個人的に思っています。

=== stateを変更するMutations, 取得するGetters
stateは、すべてのコンポーネントから取得できるので、ほいほいいろんな値を入れられてしまうと困ってしまいます。

そのため、stateをいじれるやつを決めておくようにしよう！ということで次の２つの概念が登場します。
...
//footnote[tslint][TypeScriptのlintは、eslintというJS用のlinterをts用にコンバートしたものか、ts用のtslintを利用、２つの方法があります。]
