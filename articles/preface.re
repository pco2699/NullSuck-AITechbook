...
@<kw>{テクノロジーは目的を実現する手段であり目的ではない。誰かの悩みを解決したり、やりたいことを実現するために存在する。}
...
===== AIアプリ開発ステップを知ろう
AIアプリ開発は、AIを作るためのモデリングにデータが必要不可欠です。
第1章にてAIアプリ開発の進め方を解説する理由は2つあります。

 * 適切なデータセットがなければ、作りたいAIは絶対に作れないから
 ...
 * 「信頼できる」「信頼できない」といったどちらになるか判別することができる（二値分類）
 * 「信頼できる」可能性を確率で表現することができるから（確率ごとにターゲットを複数に分類できる）
