= スコアリングモデル概論
本章では、スコアリングアプリを解説する前にスコアリングモデルについて解説します。
モデルの歴史や特徴がわかると、
@<chapref>{modeling}で実際にモデルを開発するときの楽しさも倍増するはずです。
...
しかし、このスコアリングモデル自体は機械学習やAIによって精度が高くなったものの、最新技術によって生まれたモデルではありません。

なぜ、機械学習で最もホットな分野である画像認識や自然言語処理・音声認識を本書で扱わなかったのかというと、
